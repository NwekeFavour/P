...
function UpgradePremiumPage() {
  ...
  const [phone, setPhone] = useState(""); // optional, for UI only
  ...
  const handlePaymentRedirect = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/upgrade-to-premium`,
        {
          method: "PUT",
            ...
            <form onSubmit={handlePaymentRedirect} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 outline-none text-black transition-all"
